...
def main_menu(screen, paused):
    blank = pygame.image.load(image("menu.png"))
    ...
    overlay = pygame.image.load(image("selectOverlay1.png"))
    selected = "resume"
    cooldown = 0
...
class Game:
    ...
    def loop_events(self):
        ...
        if keys[K_w] and posY > 50:
            posY -= 20
            mouse.set_pos(posX, posY)
        if keys[K_s] and posY < GameConstants.GAMEHEIGHT - 75:
            posY += 20
            mouse.set_pos(posX, posY)
        if keys[K_a] and posX > 75:
            posX -= 20
            mouse.set_pos(posX, posY)
        if keys[K_d] and posX < GameConstants.GAMEWIDTH - 75:
            posX += 20
            mouse.set_pos(posX, posY)
        ...
        for e in event.get():
            ...
            if not paused:
                ...
                if not endGame:
                    ...
                    # Handle click
                    if (e.type == KEYDOWN and e.key == K_SPACE) or (e.type == MOUSEBUTTONDOWN and
                                                                    e.button == Constants.LEFTMOUSEBUTTON):
                        mouse_down = True
                        left_button = True
                        if mouse_down and left_button:
                            ...
                            if self.timer is not None and self.timer_start == 0:
                                ...
                                for mole in self.moles:
                                    if mole.is_hit(posX, posY) == 1:  # Hit
                                        hit = True
                                        miss = False
                                    if mole.is_hit(posX, posY) == 2:  # Hit but stunned
                                        miss = False
        ...
        return clicked, hit, miss, paused

    def loop_display(self, clicked, hit, miss, paused):
        if not paused:
            gameTime, endGame = self.timerData
    ...
    def start(self):
        ...
        while self.loop:
            # Do all events
            clicked, hit, miss, paused = self.loop_events()

            # Do all render
            self.loop_display(clicked, hit, miss, paused)
